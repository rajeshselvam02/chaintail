...
function colorLevel(level: string): string {
  const RESET = '\x1b[0m';
  if (level === 'critical') return `\x1b[31m\x1b[1m${level.toUpperCase()}\x1b[0m`;
  if (level === 'high') return `\x1b[31m${level}${RESET}`;
  if (level === 'medium') return `\x1b[33m${level}${RESET}`;
  if (level === 'low') return `\x1b[32m${level}${RESET}`;
  return level;
}
