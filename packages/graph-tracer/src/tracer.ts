...
import { BitcoinTransaction } from '@chaintail/shared';
import {
  TRACE_BACKWARD,
  TRACE_FORWARD,
  CHECK_THREAT_INTEL,
...
import { RiskScorer, RiskResult, RiskFactor } from './risk-scorer';
...
export class GraphTracer {
  ...
  async trace(
    ...
    const query = direction === 'backward' ? TRACE_BACKWARD : TRACE_FORWARD;
    const { rows } = await this.db.query(query, [address, hops]);

    const nodes: TraceNode[] = rows.map((row: any) => ({
    ...
    // Persist risk score
    await this.db.query(UPSERT_ADDRESS, [
      address,
    ...
    if (riskResult.totalScore >= 60 && flagged.length > 0) {
      await this.db.query(SAVE_TRACE_RESULT, [
        address,
        flagged[0].txid,
  ...
  private async ingestAddressTransactions(address: string): Promise<void> {
    ...
    try {
      ...
      console.warn(`   Could not ingest txs: ${(err as Error).message}`);
    }
  }

  private async saveTx(tx: BitcoinTransaction): Promise<void> {
    await this.db.query(
      `INSERT INTO transactions (txid, block_height, block_hash, timestamp, fee, size, is_confirmed, raw_data)
       VALUES ($1,$2,$3,to_timestamp($4),$5,$6,$7,$8)
       ON CONFLICT (txid) DO NOTHING`,
      [tx.txid, tx.blockHeight, tx.blockHash, tx.timestamp,
       tx.fee, tx.size, tx.isConfirmed, JSON.stringify(tx)]
    );

    for (const [i, input] of tx.inputs.entries()) {
      await this.db.query(
        `INSERT INTO tx_inputs (txid, from_address, prev_txid, prev_vout, value_satoshi, vin_index)
         VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT DO NOTHING`,
        [tx.txid, input.fromAddress, input.txid, input.vout, input.valueSatoshi, i]
      );
    }

    for (const output of tx.outputs) {
      await this.db.query(
        `INSERT INTO tx_outputs (txid, to_address, value_satoshi, vout_index, script_type)
         VALUES ($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING`,
        [tx.txid, output.toAddress, output.valueSatoshi, output.n, output.scriptType]
      );
    }
  }
  ...
  private async checkThreatIntel(
    ...
    const { rows } = await this.db.query(CHECK_THREAT_INTEL, [addresses]);
    const threatMap = new Map(rows.map((r: any) => [r.address, r]));

    const flagged: TraceNode[] = [];
